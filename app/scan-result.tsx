    ...
    return (
      <SafeAreaView style={styles.safe}>
        <View style={styles.center}>
          ...
          <Text style={styles.errorTitle}>Sticker non trouvé</Text>
          <Text style={styles.errorSub}>Code: {code} #{n}</Text>
          <FmpButton label="Retour" onPress={() => router.back()} variant="ghost" />
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.toggleRow}>
        <Pressable
          ...
          style={[styles.toggleBtn, mode === 'summary' && styles.toggleActive]}
        >
          <Text style={[styles.toggleText, mode === 'summary' && styles.toggleTextActive]}>Résumé</Text>
        </Pressable>
        <Pressable
          onPress={() => setMode('anim')}
          style={[styles.toggleBtn, mode === 'anim' && styles.toggleActive]}
        >
          <Text style={[styles.toggleText, mode === 'anim' && styles.toggleTextActive]}>Album 3D</Text>
        </Pressable>
      </View>
        ...
        <View style={styles.summary}>
          ...
          {owned && (
            <View style={styles.alreadyBanner}>
              <Text style={styles.alreadyText}>⚠️ Déjà dans ta collection — ce sera un doublon</Text>
            </View>
          )}
        </View>
      ) : (
        <View style={styles.animWrap}>
          <Text style={styles.animPlaceholder}>✨ Animation 3D</Text>
          <Text style={styles.animSub}>Aperçu du sticker dans l'album</Text>
        </View>
      )}
      ...
      <View style={styles.actions}>
        <FmpButton
          label={owned ? 'Ajouter comme doublon' : 'Coller le sticker !'}
          icon={owned ? '🔄' : '✓'}
